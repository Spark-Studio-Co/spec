      ...
      >
        <div className="flex flex-row items-center gap-x-2 flex-1">
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                if (isPerformer && filteredPerformers.length > 0) {
                  const performer = filteredPerformers[0];
                  handleOptionSelect(performer.id, performer.fullname);
                } else if (!isPerformer && filteredOptions.length > 0) {
            ...
            {isPerformer ? (
              filteredPerformers.length > 0 ? (
                filteredPerformers.map((option: any, index: number) => (
                  <div
                    onClick={() =>
                      handleOptionSelect(option.id, option.fullname)
                    }
                    key={index}
                    className={`flex flex-col px-4 py-2 hover:bg-gray-100 cursor-pointer ${selected === option ? "bg-gray-50" : ""}`}
                  >
                    <span>{option.fullname}</span>
                    <span className="mt-1">{option.phone}</span>
                  </div>
                ))
              ) : (
                <span className="px-4 py-2 text-[#737373]">
                  Нет результатов
                </span>
              )
